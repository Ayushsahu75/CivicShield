...
function EvidenceStep({ form, setF, ac, gps, isVictim, nearestStation, onBack, onNext }) {
  ...
  return (
    <div style={fU(0)}>
      ...
      {/* ── LIVE CAMERA ── */}
      {mode === 'camera' && (
        <div style={{ ...sI(0), background: '#000', border: `2px solid ${ac}`, marginBottom: 14, position: 'relative', overflow: 'hidden' }}>
          <video ref={videoRef} autoPlay playsInline muted style={{ width: '100%', display: 'block', maxHeight: 280, objectFit: 'cover', background: '#111' }} />
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(0,0,0,0.6)', padding: '6px 10px' }}>
            <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#fff', opacity: 0.9 }}>📍 {gps.loading ? 'Getting GPS...' : gps.label}</div>
            <LiveClock />
          </div>
          {flash && <div style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.15)', pointerEvents: 'none' }} />}
          <canvas ref={canvasRef} style={{ display: 'none' }} />
          <div style={{ display: 'flex', gap: 1, background: '#111' }}>
            {camMode === 'photo' ? (
              <button onClick={capturePhoto} style={{ flex: 1, padding: '12px', background: ac, border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: T.sans }}>📸 Capture Photo</button>
            ) : recording ? (
              <button onClick={stopRecording} style={{ flex: 1, padding: '12px', background: T.red, border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: T.sans }}>⏹ Stop Recording</button>
            ) : (
              <button onClick={startRecording} style={{ flex: 1, padding: '12px', background: T.red, border: 'none', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: T.sans }}>🔴 Record Video</button>
            )}
            <button onClick={stopCamera} style={{ padding: '12px 16px', background: '#222', border: 'none', color: 'rgba(255,255,255,.6)', fontSize: 11, cursor: 'pointer', fontFamily: T.sans }}>✕</button>
          </div>
        </div>
      )}
...
function ReportForm({type,setCases,setScreen}){
  ...
  async function submit(){
    ...
    const STRICT_PROMPT=`You are CivicShield AI — a strict credibility scoring engine for civic incident reports in India.
...
2. location_match (0–18): mismatch=0–5, plausible=6–11, perfect fit=12–18. Deduct for GPS inconsistency.
3. time_plausibility (0–15): not provided=0–3, suspicious=4–8, logical match=9–15
4. evidence_quality (0–20): none+vague=0–3, no evidence+some detail=4–8, gallery-only max 13, live+gallery=14–17, all-live=18–20. Deduct 3–6 for delay.
5. corroboration_potential (0–12): isolated night=0–3, semi-public=4–8, busy area=9–12
...
TEMPORAL RULE: >2 hours from incident → hard reject (score < 35). >60 min evidence delay → deduct 8–15. 20–60 min → deduct 3–7.
PROXIMITY RULE: Reporter >500m from incident → hard reject (score < 35). This is non-negotiable.

CRITICAL: Fake/test/vague (<5 words, gibberish) must score below 25. No exceptions.

Respond ONLY with valid JSON, no markdown:
{"score":number,"verdict":"VERIFIED or REVIEW or REJECTED","summary":"2 sentences explaining verdict including time/location factors","signals":{"description_coherence":{"score":number,"max":20,"status":"STRONG or MODERATE or WEAK","note":"reason"},"location_match":{"score":number,"max":18,"status":"STRONG or MODERATE or WEAK","note":"reason"},"time_plausibility":{"score":number,"max":15,"status":"STRONG or MODERATE or WEAK","note":"reason"},"evidence_quality":{"score":number,"max":20,"status":"STRONG or MODERATE or WEAK","note":"source type — live/gallery"},"corroboration_potential":{"score":number,"max":12,"status":"STRONG or MODERATE or WEAK","note":"reason"},"category_match":{"score":number,"max":15,"status":"STRONG or MODERATE or WEAK","note":"reason"}},"police_action":"specific action","weak_points":["gap1","gap2","gap3"]}`;
    ...
    try{
      ...
      const ps=!hasEvidence?3:hasLiveCam?Math.min(20,14+liveProofs.length):Math.min(13,10+galleryProofs.length);
      ...
      result={
        score:tot,
        verdict:hardReject?'REJECTED':tot>=70?'VERIFIED':tot>=40?'REVIEW':'REJECTED',
        summary:hardReject
    ...
    // Only add to police dashboard if score >= 70
    if(result.score >= 70) {
      setCases(prev=>[{
  ...
  if(submitted) {
    const isRejected = !aiResult || aiResult.score < 70;
        ...
        <div style={{...sI(0),background:T.card,border:`2px solid ${T.red}`,padding:'40px'}}>
          ...
          <div style={{background:'rgba(217,26,26,.06)',border:'1px solid rgba(217,26,26,.2)',padding:'16px 20px',marginBottom:20,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <div>
              ...
              <p style={{fontSize:11,color:T.muted}}>Minimum required: <strong>70/100</strong></p>
            </div>
...
// NAV
// ══════════════════════════════════════════════════════════
function Nav({screen,setScreen,isPolice}){
  ...
  return <nav style={{position:'fixed',top:0,left:0,right:0,zIndex:400,padding:'16px 40px',display:'flex',alignItems:'center',justifyContent:'space-between',background:navBg,backdropFilter:scrolled?'blur(12px)':'none',borderBottom:scrolled?`1px solid ${T.border}`:'none',transition:'all .25s'}}>
...
export default function App(){
  ...
  return <div style={{minHeight:'100vh',background:isPolice?'#0c0c14':T.paper,paddingTop:isPolice?0:64}}>
    <Nav screen={screen} setScreen={setScreen} isPolice={!!isPolice}/>
